...
@ARCH_REGISTRY.register()
class SRFormer(pl.LightningModule):
    r""" SRFormer
        A PyTorch implement of : `SRFormer: Permuted Self-Attention for Single Image Super-Resolution`, based on Swin Transformer.
    ...
    def __init__(self,
        ...
        self.ALL_STDS = torch.Tensor([6.8789e+02, 1.1408e+01, 3.4087e+00, 3.8128e+00, 1.9669e+02, 2.2751e+02,
        ...
        self.ALL_MEANS = torch.Tensor([1.0154e+05,  2.8764e+02,  3.6334e-01, -2.1942e-01,  2.0653e+04,
        ...
        # 定义指标
        self.train_metric = nn.MSELoss()
        self.val_metric = nn.MSELoss()
        # 定义log
        self.history = {
            "loss": [], "MSE": [],
            "val_loss": [], "val_MSE": []
    ...
    def training_step(self, batch, idx):
        surf_hrrr = batch['input_hrrr']['surface'].reshape(-1, 1, 4, 320, 320).squeeze(1)
        atmo_hrrr = batch['input_hrrr']['pressure'].reshape(-1, 1, 65, 320, 320).squeeze(1)
        data_hrrr = torch.cat([surf_hrrr, atmo_hrrr], dim=1)

        surf_era5 = batch['input_era5']['surface'].reshape(-1, 1, 4, 32, 32).squeeze(1)
        atmo_era5 = batch['input_era5']['pressure'].reshape(-1, 1, 65, 32, 32).squeeze(1)
        data_era5 = torch.cat([surf_era5, atmo_era5], dim=1)

        all_means = self.ALL_MEANS.to(data_era5.device)
        all_stds = self.ALL_STDS.to(data_era5.device)
        data_hrrr = ((data_hrrr - all_means[None,:,None,None]) / all_stds[None,:,None,None])  
        data_era5 = ((data_era5 - all_means[None,:,None,None]) / all_stds[None,:,None,None])  
        sr_out = self(data_era5)

        # 计算loss
        loss = self.train_criterion(sr_out, data_hrrr)
        self.train_loss.append(loss)
        return loss
    
    def on_train_epoch_end(self):
        # 计算平均loss
        avg_train_loss = torch.cat(self.train_loss).mean()
        self.train_loss.clear()
        # 记录log
        self.history["loss"].append(avg_train_loss)
        self.history["MSE"].append(avg_train_loss)
    ...
    def validation_step(self, batch, idx):
        ...
        val_loss = self.val_criterion(sr_out, data_hrrr)
        self.val_loss.append(val_loss.unsqueeze(0))
        return val_loss

    def on_validation_epoch_end(self):
        
        avg_val_loss = torch.cat(self.val_loss).mean()

        self.history["val_loss"].append(avg_val_loss)
        self.history["val_MSE"].append(avg_val_loss)
        self.val_loss.clear()
    ...
    def configure_optimizers(self):
        ...
        StepLR = torch.optim.lr_scheduler.StepLR(optimizer, step_size=scheduler_step, gamma=scheduler_gamma)
...
class LearningCurve(Callback):
    ...
    def on_fit_end(self, trainer, module):
        ...
        for i, j in enumerate(self.names):
            ...
            plt.legend()
        plt.savefig("/home/whx/pl_SRFormer/learning_curve.png",dpi=300)


if __name__ == '__main__':
    x = torch.randn(1, 69, 179, 105)

    model = SRFormer(img_size=224, upsampler = 'pixelshuffledirect',in_chans=69,
                    window_size=2,depths=[6, 6, 6, 6, 6, 6], 
                    num_heads=[6, 6, 6, 6, 6, 6], upscale=10)
    y = model(x)
    print(y.shape)
    print(model.flops()/1e9)#单位为G

    #生成代码，计算flops
    from thop import profile
    flops, params = profile(model, inputs=(x,))
    print('flops: ', flops/1e9) #单位为G
